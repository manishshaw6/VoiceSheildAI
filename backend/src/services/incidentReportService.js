...
export async function generateIncidentReport({ analysisId, user }) {
  ...
  const rawReportPayload = {
    ...
    generatedAt,
    incidentTimestamp: analysisRow.timestamp || raw.timestamp || null,
    reportingUser: {
      id: user.id,
    ...
    riskAssessment: {
      score: risk.score,
      level: risk.level,
      threatCategory: analysisRow.threat_category || convIntel?.threat_assessment?.threat_category || 'Not determined',
    ...
    voiceAuthenticity: {
      provider: deepfake.provider || null,
      verdict: deepfake.verdict || deepfake.classification || 'NOT_AVAILABLE',
    ...
    conversationIntelligence: {
      ...
      attackCategory: convIntel?.threat_assessment?.threat_category || analysisRow.threat_category || 'Not determined',
    ...
    evidenceTimeline: (raw.timeline || [])
      .filter(item => Number.isFinite(Number(item.start ?? item.startTime ?? item.timestamp)))
      .map(item => ({
        timestampSeconds: Number(item.start ?? item.startTime ?? item.timestamp),
        label: item.label || item.indicators?.[0] || (item.flagged ? 'Flagged transcript segment' : 'Transcript segment'),
        excerpt: item.text || item.evidence || null,
    ...
    userExposure: {
      ...
      riskAfterCall: convIntel?.victim_exposure?.risk_after_call || 'NOT_DETERMINED'
    },
    ...
    integrityInformation: {
      ...
      verificationStatus: 'REPORT SIGNATURE APPLIED',
      version: 'VoxShield v1.0.0 (HMAC-SHA256)'
    },
    disclaimer: 'VOXSHIELD SIGNED ANALYTICAL REPORT. The signature verifies report integrity, not the truth of caller claims or the correctness of an AI assessment. This is not a government certification, police determination, legal opinion, or proof of wrongdoing.'
  };
...
<body>
  <div class="container">
    ...
    // SendGrid/Gmail acknowledgement means the provider accepted the message;
    // it is not proof of inbox delivery. Provider webhooks may later advance the
    // delivery status to DELIVERED or BOUNCED.
    const acceptedStatus = sendResult.deliveryStatus || 'ACCEPTED';
    deliveryMeta.status = acceptedStatus;

    // Keep the legacy report status for API/Android compatibility while exposing
    // the truthful provider state in delivery metadata.
    await query.run(`
      UPDATE incident_reports
