...
const THREAT_PATTERNS = [
  ...
  // 2. Financial Coercion & Urgent Wire Transfer
  {
    ...
    severity: 30,
    patterns: [
      /\b(transfer\s*money|send\s*money|pay\s*immediately|upi\s*payment|google\s*pay|phonepe|paytm\s*karo|transfer\s*funds|wire\s*transfer)\b/i,
  ...
  // 3. Authority Impersonation & Legal Threat (CBI, Police, Digital Arrest)
  {
  ...
  // 6. Account Blockage & Fake KYC Expiry
  {
    ...
    severity: 25,
    patterns: [
      /\b(account\s*(blocked|suspended|deactivated|freeze|expired)|kyc\s*(update|mandatory|expired))\b/i,
