...
const LiveStreamMonitor = forwardRef(function LiveStreamMonitor(
  ...
  // VOIP integration refs
  const ownsAudioStreamRef = useRef(false);
  const sessionIdRef = useRef(null);

  // WebAudio telemetry refs
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // ANIMATION: Smooth EWMA lerp for score display
  // ═══════════════════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // WEBAUDIO REAL-TIME TELEMETRY TRACKER
  // Pitch + RMS + VAD
  // ═══════════════════════════════════════════════════════════════════════
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn(
        'WebAudio telemetry init warning:',
        e
      );
    }
    ...
    try {
      ...
      socket.onopen = async () => {
        try {
          ...
          // Use provided VOIP/LiveKit stream when available.
          // Otherwise request the browser microphone.
          const stream =
            providedStream ||
          ...
          // The remote LiveKit path deliberately disables
          // browser speech recognition when requested.
          // Web Speech listens to the local microphone,
          // not necessarily the subscribed remote track.
          const SpeechRec =
            ...
            recognition.onresult = (
              ...
              for (
                ...
                if (
                  ...
                  // Low-latency threat escalation
                  const lower =
                    transcriptPiece.toLowerCase();

                  const isThreatKeyword =
                    /otp|pin|password|bank|wire|transfer|police|cbi|arrest|urgent|card|verify|account|credential/i.test(
                      lower
                    );
  ...
  const stopLiveMonitor = (
    ...
    // Stop local microphone hardware immediately
    if (
      mediaRecorderRef.current &&
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // AUDIO STREAM AUTO START / HOT SWAP
  // ═══════════════════════════════════════════════════════════════════════
  ...
  useEffect(() => {
    if (
      ...
      // Hot-swap stream into active MediaRecorder
      if (
        mediaRecorderRef.current &&
        mediaRecorderRef.current.state !==
          'inactive'
  ...
  return (
    <div className="live-stream-panel">
      ...
      <div className="stream-header">
        ...
        <h3>
          Live Call Shield & Threat
          Interceptor
        </h3>

        <p>
          Real-time conversational threat
          stream analyzing speech patterns,
          OTP harvesting, urgency pressure,
      ...
      {cloneWarning && (
        <div className="clone-alert-banner">
          ...
          <span>
            ...
            Live voice matches enrolled
            speaker identity with synthetic
            speech signatures!
          </span>
      ...
      <div className="live-metrics-grid">
        ...
        <div className="live-metric-card signals-card">
          ...
          <div className="metric-title">
            ACTIVE DETECTED THREAT
            INDICATORS ({detectedSignals.length})
          </div>
