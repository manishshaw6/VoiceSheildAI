...
vec3 hue2rgb(float h){
    ...
    float b = clamp(abs(h*6.0 - 5.0) - 1.0, 0.0, 1.0);
    return vec3(r, g, b);
}
...
export interface SiriWaveProps extends Omit<React.HTMLAttributes<HTMLCanvasElement>, "children"> {
...
export function SiriWave({
  ...
  return (
    <canvas
      ref={canvasRef}
      className={`block rounded-[20px] bg-black ${className || ""}`}
